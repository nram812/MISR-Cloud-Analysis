...
cond3=(pred[cond2]==0)&(cf_res[cond2]>0.7)
...
mask_for_fitting=~np.isnan(a)
params=curve_fit(f,x[0][1:][mask_for_fitting],a[mask_for_fitting])
fitted_residuals=f(h_i[cond2],*params[0])
final_residuals=residual-fitted_residuals
...
fig,ax=plt.subplots(1,2)
#a[a<=2]=np.nan
a2, x, y = binned_statistic_dd(sample=np.vstack([cf_res[cond2],residual]).T, values=residual,
                              statistic='count', bins=[np.arange(0, 1, 0.005),np.arange(-0.2,0.2,0.01)])
